...
const GameBoard = () => {
  ...
  const { pause, algorithm, maxDepth, numberOfIterations, simulationDepth } =
  ...
  const handleKeyDown = (event) => {
    if (
      ["ArrowUp", "ArrowDown", "ArrowRight", "ArrowLeft"].indexOf(event.key) >
      -1
    ) {
  ...
  useEffect(() => {
    if (gameOver) {
      ...
      if (
        maxTile > bestRecord.tile ||
        (maxTile === bestRecord.tile && steps < bestRecord.steps)
      ) {
  ...
  return (
    <div className={`game-board`}>
      ...
      {board.map((row, indexRow) => {
        return row.map((tile, indexColumn) => {
